...
import jsettlers.logic.algorithms.path.astar.normal.AStarMinHeap;
import jsettlers.logic.algorithms.path.astar.normal.IAStarHeapable;
import jsettlers.logic.algorithms.path.astar.supergrid.ISuperGridAStarGrid.IBlockedChangedListener;
...
public final class SuperGridAStar implements IAStarHeapable, IBlockedChangedListener {
	...
	private static final boolean SET_DEBUG_COLORS = false;
	 ...
	 */
	public SuperGridAStar(ISuperGridAStarGrid grid, short tileWidth, short tileHeight) {
		this.grid = grid;
	...
	public ArrayList<ShortPoint2D> findPath(short tileSx, short tileSy, short tileTx, short tileTy) {
		openClosed.clear();
		open.clear();
		...
		for (int i = 0; i < 4; i++) {
			...
			if (isInBounds(targetX, targetY)) {
				setColor(targetX, targetY, Color.ORANGE);
		...
		for (int i = 0; i < 4; i++) {
			...
			if (isInBounds(neighborX, neighborY)) {
				setColor(neighborX, neighborY, Color.ORANGE);
		...
		MAIN_LOOP: while (!open.isEmpty()) {
			...
			setColor((currIdx % width), (currIdx / width), Color.BLUE);
		...
		ArrayList<ShortPoint2D> path = new ArrayList<ShortPoint2D>();
		while (foundTarget >= 0) {
	...
	private final void setColor(int x, int y, Color color) {
		if (SET_DEBUG_COLORS) {
			grid.setDebugColor(x * STEP_WIDTH, y * STEP_WIDTH, color);
		}
	}
