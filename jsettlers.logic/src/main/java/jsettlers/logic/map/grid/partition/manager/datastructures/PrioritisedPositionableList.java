 ...
 */
public class PrioritisedPositionableList<P extends Enum, T extends ILocatable> implements Serializable {
	private final PositionableList<T>[] lists;

	public PrioritisedPositionableList(int numberOfPriorities) {
	...
	public T getObjectAt(ShortPoint2D position, P minimumIncludedPriority) {
		for (int i = lists.length - 1; i >= minimumIncludedPriority.ordinal(); i--) {
			T object = lists[i].getObjectAt(position);
			if (object != null) {
				return object;
			}
		}
	...
	public void moveObjectsAtPositionTo(ShortPoint2D position, PrioritisedPositionableList<P, T> otherList, IMovedVisitor<T> movedVisitor) {
		for (int i = lists.length - 1; i >= 0; i--) {
			lists[i].moveObjectsAtPositionTo(position, otherList.lists[i], movedVisitor);
		}
	}

	public void addAll(PrioritisedPositionableList<P, T> otherList) {
		for (int i = lists.length - 1; i >= 0; i--) {
